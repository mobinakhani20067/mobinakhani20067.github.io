...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addBotMessage(text) {
        if (chatMessages) {
            chatMessages.innerHTML += `<div class="message bot-message">${text}</div>`;
    ...
    if (sendMessageBtn && chatInput) {
        const handleChatSend = function() {
            const msg = chatInput.value.trim();
            if (msg) {
                addUserMessage(msg);
                chatInput.value = '';
                setTimeout(() => {
                    addBotMessage("پیام شما دریافت شد. تیم پشتیبانی به زودی با شما تماس می‌گیرد.");
                }, 500);
            }
        };
    ...
    quickReplyBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (msg && chatInput) {
                ...
                document.getElementById('sendMessage')?.dispatchEvent(sendEvent);
            }
        });
    });
});
