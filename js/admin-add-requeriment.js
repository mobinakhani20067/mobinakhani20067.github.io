...
function showManualLogs() {
        ...
        <div class="log-modal-overlay" id="logModalOverlay">
            <div class="log-modal-content">
                <div class="log-modal-header">
                    <h3>تاریخچه تکمیل دستی نیازمندی‌ها</h3>
                    <span class="log-close-btn" onclick="closeLogModal()">&times;</span>
                </div>
    ...
    manualCompleteLogs.forEach(log => {
        logsHtml += `
            <tr>
                ...
                <td style="font-weight: bold;">${escapeHtml(log.needTitle)}</td>
                <td style="color: #27ae60;">${log.remainingAmount.toLocaleString()} تومان</td>
    ...
    logsHtml += `
                ...
                <div style="margin-top: 15px; text-align: center;">
                    <button class="clear-logs-btn" onclick="clearAllLogs()">پاک کردن همه لاگ‌ها</button>
                    <button class="close-modal-btn" onclick="closeLogModal()">بستن</button>
                </div>
            </div>
        </div>
    `;
...
function closeLogModal() {
    const modal = document.getElementById('logModalOverlay');
    if (modal) modal.remove();
}

function clearAllLogs() {
    if (confirm("آیا از پاک کردن تمام لاگ‌های تکمیل دستی اطمینان دارید؟ این عمل غیرقابل بازگشت است.")) {
        manualCompleteLogs = [];
        saveManualLogs();
        closeLogModal();
        showToast("تمام لاگ‌ها پاک شدند", false);
    }
}
function numberToWords(num) {
    if (num === 0 || isNaN(num)) return "صفر تومان";
...
function updateTargetWords() {
    ...
    if (!isNaN(amount) && amount > 999999999) {
        ...
        targetWords.innerHTML = "⚠️ مبلغ بیش از حد مجاز (حداکثر ۹۹۹,۹۹۹,۹۹۹ تومان)";
        targetWords.style.color = "#c0392b";
        targetWords.style.background = "#fee";
        return;
    }
    ...
    if (!isNaN(amount) && amount > 0) {
        ...
        targetWords.style.color = "#2c7a4a";
        targetWords.style.background = "#e8f5e9";
    } else {
...
function renderTable() {
    ...
    pageData.forEach(need => {
        ...
        if (isAlmostComplete) {
            actionsHtml += `
                <button class="action-btn complete-btn" onclick="forceCompleteNeed(${need.id}, ${remaining})">
        ...
        let warningBadge = '';
        if (isAlmostComplete) {
            warningBadge = `<span class="warning-badge">باقی مانده کمتر از ۱۰ هزار تومان</span>`;
        }
...
function saveNeed() {
    ...
    if (targetAmount > 999999999) {
        showToast("مبلغ هدف نمی‌تواند بیشتر از ۹۹۹,۹۹۹,۹۹۹ تومان باشد", true);
        return;
    }
    ...
    if (currentEditId === null) {
        ...
        showToast("نیازمندی \"" + title + "\" با موفقیت اضافه شد", false);
    } else {
        ...
        if (index !== -1) {
            ...
            showToast("نیازمندی \"" + title + "\" با موفقیت ویرایش شد", false);
        }
    }
...
function deleteNeed(id) {
    ...
    if (need) {
        if (confirm("آیا از حذف نیازمندی \"" + need.title + "\" اطمینان دارید؟")) {
            ...
            saveToLocalStorage();
            filterData();
            showToast("نیازمندی \"" + need.title + "\" حذف شد", false);
        }
    }
}
...
document.getElementById('showLogsBtn').addEventListener('click', showManualLogs);
